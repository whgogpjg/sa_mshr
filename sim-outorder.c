...
/* mshr */
static int mshr_num_entry;
static int mshr_max_targets;
...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  /* mshr */
  opt_reg_int(odb, "-mshr:num_entry",
        "number of mshr entry (if 0, mshr disabled)",
        &mshr_num_entry, /* default */0,
        /* print */TRUE, /* format */NULL);

  opt_reg_int(odb, "-mshr:max_targets",
        "Number of max targets per each mshr entry",
        &mshr_max_targets, /* default */2,
        /* print */TRUE, /* format */NULL);
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      	cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       ...
			       dl1_access_fn, /* hit lat */cache_dl1_lat, mshr_num_entry, mshr_max_targets);
      ...
      else
	{
	  ...
	  cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   dl2_access_fn, /* hit lat */cache_dl2_lat, mshr_num_entry, mshr_max_targets);
	}
    }
  ...
  if (!mystricmp(cache_il1_opt, "none"))
	...
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       il1_access_fn, /* hit lat */cache_il1_lat, 0, 0);
      ...
      else
	{
	  ...
	  cache_il2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   il2_access_fn, /* hit lat */cache_il2_lat, 0, 0);
	}
    }
	  ...
	  if ((MD_OP_FLAGS(LSQ[LSQ_head].op) & (F_MEM|F_STORE))
	      == (F_MEM|F_STORE))
	    {
		 ...
		 try to get a store port (functional unit allocation) */
	      ...
	      if (fu && LSQ[LSQ_head].mem_ready <= sim_cycle) // mshr
		{
