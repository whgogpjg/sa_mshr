...
cache_mshr*
cache_mshr_create(unsigned int mshr_num_entry, unsigned int mshr_max_targets)
{
  if (mshr_num_entry == 0)
	  return NULL;

  if (mshr_max_targets == 0)
    fatal("mshr max targets must be larger than 0");


  cache_mshr* mshr = (cache_mshr *)calloc(1, sizeof(cache_mshr));
  if (!mshr)
    fatal("out of virtual memory");

  mshr->num_entry = mshr_num_entry;
	mshr->free_entry = mshr_num_entry;
  mshr->max_targets = mshr_max_targets;
  mshr->entries = (mshr_entry *)calloc(mshr_num_entry, sizeof(mshr_entry));
  if (!mshr->entries)
    fatal("out of virtual memory");
...
void
refresh_mshr_entry(cache_mshr *mshr, tick_t now)
{
	for (unsigned int i = 0; i < mshr->num_entry; i++)
	{
		if (mshr->entries[i].valid && mshr->entries[i].ready <= now)
		{
			mshr->entries[i].valid = FALSE;
			mshr->free_entry++;
		}
  }
}
...
mshr_entry*
search_mshr_hit(cache_mshr *mshr, md_addr_t addr)
{
	mshr->stats.search++;
	for (unsigned int i = 0; i < mshr->num_entry; i++)
	{
		if (mshr->entries[i].valid && mshr->entries[i].block_addr == addr)
		{
			mshr->stats.hit++;
...
bool_t
add_mshr_target(cache_mshr *mshr, mshr_entry *mshr_entry)
{
	if (mshr_entry->num_targets < mshr->max_targets)
  {
		mshr_entry->num_targets++;
		return TRUE;
  }
	mshr->stats.target_full++;
	return FALSE;
}

bool_t
has_mshr_free_entry(cache_mshr *mshr)
{
	if (mshr->free_entry)
		return TRUE;
	mshr->stats.entry_full++;
	return FALSE;
}

void
add_mshr_entry(cache_mshr *mshr, md_addr_t addr, tick_t ready)
{
	for (unsigned int i = 0; i < mshr->num_entry; i++)
  {
		if (mshr->entries[i].valid == FALSE)
    {
			...
			mshr->entries[i].block_addr = addr;
			mshr->entries[i].ready = ready;
			mshr->entries[i].num_targets = 1;

			if (mshr->free_entry == 0)
				fatal("mshr free entry");
			mshr->free_entry--;
			mshr->stats.alloc++;
			return;
    }
...
cache_create(char *name,		/* name of the cache */
	     ...
	     unsigned int hit_latency,	/* latency in cycles for a hit */
             unsigned int mshr_num_entry,	/* number of mshr entry */
             unsigned int mshr_max_targets)    /* number of mshr targets */
{
  struct cache_t *cp;
  struct cache_blk_t *blk;
  int i, j, bindex;
  ...
  cp->mshr = cache_mshr_create(mshr_num_entry, mshr_max_targets);
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  if (cp->mshr) {
		mshr_stats *stats = &cp->mshr->stats;

     sprintf(buf, "%s.mshr_alloc", name);
     stat_reg_counter(sdb, buf, "total number of mshr full events", &stats->alloc, 0, NULL);
     sprintf(buf, "%s.mshr_search", name);
     stat_reg_counter(sdb, buf, "total number of mshr full events", &stats->search, 0, NULL);
     sprintf(buf, "%s.mshr_hit", name);
     stat_reg_counter(sdb, buf, "total number of mshr full events", &stats->hit, 0, NULL);
     sprintf(buf, "%s.mshr_target_full", name);
     stat_reg_counter(sdb, buf, "total number of mshr full events", &stats->target_full, 0, NULL);
     sprintf(buf, "%s.mshr_entry_full", name);
     stat_reg_counter(sdb, buf, "total number of mshr full events", &stats->entry_full, 0, NULL);
     ...
     stat_reg_counter(sdb, buf, "total number of mshr hits under misses", &stats->hits_under_misses, 0, NULL);
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  md_addr_t bofs = CACHE_BLK(cp, addr);
	md_addr_t tagset = CACHE_TAGSET(cp, addr); // for mshr
...
//    fatal("cache: access error: access spans block, addr 0x%08x", addr);
  ...
  cp->misses++;
	...
	if (cp->mshr)
	{
		refresh_mshr_entry(cp->mshr, now);

		mshr_entry *mshr_entry = search_mshr_hit(cp->mshr, tagset);
		if (mshr_entry != NULL)
		{
			if (add_mshr_target(cp->mshr, mshr_entry))
				return mshr_entry->ready - now;
			else
				return MSHR_FULL;
		}

		if (has_mshr_free_entry(cp->mshr) == FALSE)
			return MSHR_FULL;
  }
  ...
  if (cp->hsize)
	...
	if (cp->mshr)
		add_mshr_entry(cp->mshr, tagset, now+lat);
