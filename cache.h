...
typedef struct _mshr_stats
{
  counter_t alloc;
  counter_t search;
  counter_t hit;
  counter_t target_full;
  counter_t entry_full;
} mshr_stats;

typedef struct _mshr_entry
{
  bool_t valid;
  tick_t ready;
  md_addr_t block_addr;
  unsigned int num_targets;
} mshr_entry;

typedef struct _cache_mshr
{
...
cache_mshr* cache_mshr_create(unsigned int mshr_num_entry, unsigned int mshr_max_targets);
void refresh_mshr_entry(cache_mshr *mshr, tick_t now);
mshr_entry* search_mshr_hit(cache_mshr *mshr, md_addr_t addr);
bool_t add_mshr_target(cache_mshr *mshr, mshr_entry *mshr_entry);
bool_t has_mshr_free_entry(cache_mshr *mshr);
void add_mshr_entry(cache_mshr *mshr, md_addr_t addr, tick_t ready);
